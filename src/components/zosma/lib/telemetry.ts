...
async function initSentry(): Promise<void> {
	...
	try {
		...
		// my-hermes-rs 不内置 Sentry 客户端（不引入 @sentry/react 依赖）：
		// 崩溃上报由 hermes/dsh 引擎侧负责，这里只保留 DSN 占位，避免
		// 动态 import 在构建期被 Rollup 解析失败。
		console.debug("[telemetry] sentry dsn configured but client not bundled:", sentryDsn);
