import type { ChatMessage, ToolCallInfo } from "@/components/zosma/types";
...
} from "@/components/zosma/types/pi-events";
import { $providerConfig, activeModelIdOf } from "@/store/providerConfig";
import {
	startAiMessage,
	appendAiDelta,
	appendAiThinking,
...
export function useDSHStream() {
	...
	const startStream = useCallback(async (text: string) => {
		...
		// 桥接到 $liveMessages（默认/dsh 风格共用会话显示；切风格不丢对话）
		appendUserMessage(text);

		const channel = new Channel<PiEvent>();
			// 发送时的活跃会话与环境代数：事件到达时已切走 → 丢弃迟到事件（防跨会话/跨环境污染）
			const sidAtSend = $activeSessionId.get();
			void sidAtSend;
			const epochAtSend = $envEpoch.get();

		// —— dsh 会话桥接到 $liveMessages：assistant 文本流写入共享 store ——
		let bridgeId = "";
		let bridgeAcc = "";
		...
		channel.onmessage = (event: PiEvent) => {
			try {
				if ($envEpoch.get() !== epochAtSend) return; // 环境/会话已切换：丢弃本流事件
				// token-meter usage / 子代理事件（不在 PiEvent 联合里，先拦截）
				const evtType = (event as unknown as { type?: string }).type;
				if (evtType === "usage") {
					recordUsage(
						(event as unknown as { usage?: { inputTokens?: number; outputTokens?: number; cacheReadTokens?: number; reasoningTokens?: number } }).usage ?? {},
					);
					return;
				}
				if (evtType === "subagent.started") {
					const p = (event as unknown as { params?: Record<string, unknown> }).params ?? {};
					addSubagent(
				...
				if (evtType === "subagent.finished") {
					const p = (event as unknown as { params?: Record<string, unknown> }).params ?? {};
					const id = String(p.childSessionId ?? "");
					if (id) updateSubagentStatus(id, "completed");
					return;
				}
				if (evtType === "compaction_summary") {
					// dsh compaction/summary → 压缩标记（dsh 风格 CompactionRow 显示）
					const d = (event as unknown as { payload?: Record<string, unknown> }).payload ?? {};
					const seqs = d.shadowedSeqs;
					const count = Array.isArray(seqs) ? seqs.length : null;
					const tokens = typeof d.shadowedTokenCount === "number" ? d.shadowedTokenCount : null;
					const summaryBlocks = Array.isArray(d.summary) ? d.summary : [];
					const summary = summaryBlocks
						.filter((b) => (b as { type?: string }).type === "text")
						.map((b) => (b as { text?: string }).text ?? "")
						.join("");
					if (count !== null && tokens !== null) {
						appendCompactionMessage({ count, tokens, ...(summary ? { summary } : {}) });
					} else if (summary) {
						appendCompactionMessage({ count: 0, tokens: 0, summary });
					}
					return;
				}
				if (evtType === "user_question") {
					// 引擎提问（ask_user_question）→ 全局待答问题（MainPanel 层提问卡渲染）
					const p = (event as unknown as { params?: { rpcId?: string; questions?: unknown[] } }).params ?? {};
					const questions = Array.isArray(p.questions)
						? (p.questions as { id?: string; question?: string; detail?: string; header?: string; options?: { label?: string; description?: string }[]; multiSelect?: boolean }[])
					...
					if (p.rpcId && questions.length > 0) {
						setPendingQuestions({ rpcId: p.rpcId, questions, askedAt: Date.now() });
					}
					return;
				}
				switch (event.type) {
					case "message_update": {
						...
						if (msgEvent.message?.model || msgEvent.message?.provider) {
							...
							switch (ame.type) {
								case "thinking_delta":
									appendAiThinking(bridgeId, ame.delta ?? "");
									dispatch({ type: "THINKING_DELTA", delta: ame.delta });
									break;
								case "text_delta":
									bridgeAcc += ame.delta ?? "";
									appendAiDelta(bridgeId, ame.delta ?? "");
					...
					case "tool_execution_start": {
						...
						// dsh 风格：工具调用挂到当前 AI 消息（DshChat 工具行数据源）
						appendAiTool(bridgeId, {
							id: te.toolCallId,
							name: te.toolName || "tool",
							args: (te.args ?? {}) as Record<string, unknown>,
							status: "running",
						});
						// dsh todo 工具 → 真实待办 store
						if (te.toolName === "todo") {
							const args = (te.args ?? {}) as Record<string, unknown>;
					...
					case "tool_execution_update": {
						...
						updateAiTool(bridgeId, te.toolCallId, {
							status: "running",
					...
					case "tool_execution_end": {
						...
						const resultText = (te.result?.content || []).map((c) => c.text).join("");
						updateAiTool(bridgeId, te.toolCallId, {
					...
					case "message_end": {
						...
						if (error) {
							if (bridgeOpen) {
								finalizeAiMessage(bridgeId);
								bridgeOpen = false;
							}
							// 错误由下方 ErrorBanner（STREAM_ERROR）展示，不重复插入系统消息
						} else if (bridgeOpen) {
							finalizeAiMessage(bridgeId, bridgeAcc || undefined);
							bridgeOpen = false;
						}
					...
					case "error": {
						...
						// 错误由 ErrorBanner（STREAM_ERROR）展示，不重复插入系统消息
						dispatch({
							type: "STREAM_ERROR",
		...
		try {
			// 携带设置页选中的 provider/model（自定义模型真正作用于 dsh 运行时）
			...
			await invoke("send_prompt", { text, ch: channel, provider, model });
		} catch (err) {
			dispatch({
				type: "STREAM_ERROR",
