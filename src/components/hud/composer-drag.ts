...
// mirach 适配：hermes 的 beginMove/moveBy/endMove 配对协议是 Electron 主进程
// 光标采样；Tauri 的 start_dragging 把拖拽整体交给原生 WM，一次调用即等价。
// workspaceTransfer 是 X11 虚拟桌面特性，Windows 不涉及。
const hudBeginMove = (): void => {
  void invoke('hud_begin_move').catch(() => {})
}

function setWorkspaceTransfer(_transferring: boolean): void {
  /* mirach: X11 虚拟桌面跟随无对应物，空实现保结构 */
}

function moveHud(_state: PressState): void {
  /* mirach: start_dragging 已接管拖拽（armGrab 触发一次），无逐帧 move */
}
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        /* endMove: start_dragging 由 WM 在指针释放时收尾 */
      }
