...
export function useHudGlass(rootRef: RefObject<HTMLElement | null>, backing: boolean): void {
  useEffect(() => {
    ...
    // mirach 适配：hermes 的 setFrost 是 macOS vibrancy / Win11 DWM backdrop
    // 的原生材质开关；Tauri 窗口未启用窗口级磨砂，空实现——毛玻璃由 CSS
    // 半透明面板承担，门控逻辑保持原样。
    const setFrost: ((showing: boolean) => void) | null = (showing: boolean) => {
      void showing
    }
