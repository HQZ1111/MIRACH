...
export function useHudClickThrough(rootRef: RefObject<HTMLElement | null>): void {
  useEffect(() => {
    ...
    // mirach 适配：Electron setIgnoreMouseEvents → Tauri set_ignore_cursor_events
    const setIgnoreMouse = (ignore: boolean) => {
      void invoke('hud_set_ignore_mouse', { ignore }).catch(() => {})
    }
    ...
    // — leaving the bar permanently click-through. mirach：Windows/macOS 主路
    // 径已覆盖，此 Linux 推送通道无对应物。
