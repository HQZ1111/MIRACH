...
// mirach 适配：hermes 经 translateNow 走 i18n；mirach 无对应词条，
// 错误摘要改为内联中文（结构/规则保持 hermes 原样）。
  ...
  },
  {
    ...
    summarize: msg =>
      /ELEVENLABS_API_KEY not set/i.test(msg)
        ? '语音转写需要配置 ElevenLabs Key'
        : 'ElevenLabs 拒绝了 API Key'
  },
  {
    test: msg => /method not allowed/i.test(msg),
    summarize: () => '请求方法不被允许（接口版本不匹配？）'
  },
  {
