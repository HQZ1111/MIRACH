...
- **原因**：二维码里的 192.168.x.x 是内网地址，只在家庭路由器内可路由——手机
  换网（流量/别家 WiFi）必然打不开；跨网需要公网入口或虚拟局域网。同网打不开
  的常见原因：开关没重启生效、Windows 防火墙拦 Node.js。
- **net.access 升级**（agent-sidecar/plugins.ts）：逐网卡分类（lan / tailscale
  CGNAT 100.64/10 / zerotier）+ **逐 IP 探测核心 web 面连通性**（net.connect
  600ms）→ 前端直接显示每个地址"可达 / 核心未监听"。
- **前端分组渲染**：「本局域网」与「虚拟网络（跨网可用）」分组；点击行选中 =
  二维码跟随；不可达时红条提示（开开关+重启+防火墙放行）。
- **跨网推荐**：Tailscale（免费）——电脑+手机同账号即组成加密虚拟局域网，
  100.x 地址任何网络可访问；面板自动识别 Tailscale/ZeroTier 网卡并列出。
  直接公网暴露（端口转发）不安全，不支持。
...
1. **插件市场发现 ✅**：插件管理器"安装"页签新增 npm registry 搜索
   （fetch_text 绕 CORS 拉 registry.npmjs.org/-/v1/search，默认搜 dsh），
   结果直接点装；dsh-agent-rp 在 npm 上不存在（404），待用户提供来源。
2. **群聊 v1 ✅**（酒馆群组聊天的 mirach 等价）：
   - 群聊 = 聊天环境团队里的伪成员（id grp-<ts>，左栏直接可见），参与者 +
     回复策略（全员依次 / 轮流发言）存 store/groups.ts（localStorage）。
   - 发送：@点名优先 → 策略选应答者，逐个走各自成员会话（persona/酒馆预设
     照常生效，酒馆成员自动登记 session-bindings），prompt 注入最近 12 条群聊
     记录 + "以「X」身份回应"约束；回复带成员署名（ChatMessage.from）。
   - 创建/管理：智能体团队（聊天标签）→ 群聊区：新建（名称+参与者多选+策略）、
     打开（事件 → AppLayout 展开群聊子面板）、删除（伪成员+定义一并清）。
   - 渲染：MemberChatPanel 消息带 from 署名（头像/名字逐条区分）；状态栏卡/
     剧情选项按钮在群聊同样生效。
- 已知边界：群聊上下文靠 prompt 注入最近 12 条（各成员仍保有自己会话的长期
  记忆）；轮流模式的轮次按用户消息数轮转。
- tsc 通过；本批无 Rust 变更，刷新前端即生效。
...
用户反馈五点全部落地：
1. **角色库扩容+分类**：8 个自编角色 → 22 个、6 分类（陪伴闲聊/古风武侠/科幻奇幻/
   悬疑推理/学习工作/生活实用），分类 pills + 搜索框筛选；旧 8 个按用户指示删除
   （概念好的并入新库）。
2. **tavern-lite 说明改写**：明确"没有任何角色人设——导入只是通用扮演空壳成员，
   一般不用导入"；它只是酒馆插件自带基础预设（目录名兼容旧版）。
3. **角色卡文件选择改原生对话框**：plugin-dialog open（defaultPath=用户主目录，
   JSON 过滤，可多选），替换 HTML input（之前跳转位置不对）；读文件走 read_file。
4. **弹窗加右上角 X 关闭按钮**。
5. **其他环境的提示行删除**（导入入口直接只在聊天标签显示，无多余提示）。
6. **金融环境改名**：种子 "金融写作"→"金融"（cwd finance-writing→finance），
   migrate 对存量数据做精确旧名迁移（用户自定义名不动）。
- tsc 通过。

# 第五批：酒馆导入界面重做（2026-08-31 深夜；用户反馈驱动）

用户反馈：① 其他环境也出现导入入口；② 导入没有角色表和说明；③ 问 tavern-lite
是什么、文件导入是什么、去哪找文件；④ 要求角色做前端选择+可修改。落地：

- **内置角色库**（`src/lib/tavern-characters.ts`，8 个成品角色：苏晚晴/墨先生/
  阿狸/夜枭/凯拉/老白/讲述者/艾琳教授，完整人设）——导入弹窗第一标签，
  无需任何文件；每个角色可点「修改」展开人设草稿，改完再导入。
- **导入弹窗三标签**：角色库 / 酒馆预设 / 角色卡文件。预设页标注 tavern-lite =
  酒馆插件自带的基础预设（无角色空白扮演底座，目录名兼容旧版，非 bug）；
  文件页说明 JSON 来源（SillyTavern 导出/角色分享站/自写）并给最小结构示例。
- **导入入口只在聊天环境标签显示**（viewEnv === TAVERN_MEMBER_ENV）；
  其他环境标签显示一行说明"酒馆角色固定导入到聊天环境"。
- tsc 通过。
