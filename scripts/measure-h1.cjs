...
const PORT = 9334, URL = "http://localhost:1420/?win=login";
const USER_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "edge-m-"));
...
(async () => {
  ...
  console.log(JSON.stringify(await evalJs(`(() => {
    ...
    const r = el => { const b = el.getBoundingClientRect(); return { x: Math.round(b.x), w: Math.round(b.width), y: Math.round(b.y), h: Math.round(b.height) }; };
    return {
      h1: { ...r(h1), fs: getComputedStyle(h1).fontSize, ls: getComputedStyle(h1).letterSpacing, font: getComputedStyle(h1).fontFamily.split(",")[0] },
      h2: r(h2),
      panel: r(panel),
      panelContentW: panel.clientWidth,
      overflowsPanel: r(h1).x + r(h1).width > r(panel).x + panel.clientWidth,
    };
  })()`), null, 2));
