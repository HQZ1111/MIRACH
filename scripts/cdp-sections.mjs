const WS = process.argv[2];
const ws = new WebSocket(WS);
let seq = 0;
const pending = new Map();
function send(method, params = {}) {
  return new Promise((resolve, reject) => { const id = ++seq; pending.set(id, { resolve, reject }); ws.send(JSON.stringify({ id, method, params })); });
}
async function evalJs(expression) {
  const r = await send("Runtime.evaluate", { expression, returnByValue: true, awaitPromise: true });
  if (r.exceptionDetails) return { threw: r.exceptionDetails.text + " " + (r.exceptionDetails.exception?.description ?? "").slice(0, 300) };
  return r.result?.result?.value;
}
ws.onmessage = (ev) => {
  const m = JSON.parse(ev.data);
  if (m.id && pending.has(m.id)) { pending.get(m.id).resolve(m); pending.delete(m.id); }
};
ws.onopen = async () => {
  await send("Runtime.enable");
  for (const sec of ["通用设置", "模型", "插件", "Agent 预设"]) {
    await evalJs(`(() => { const d = document.querySelector('.settings-dropdown'); const b = [...d.querySelectorAll('button')].find(x => x.textContent.trim() === '${sec}'); b?.click(); return 'ok'; })()`);
    await new Promise((r) => setTimeout(r, 2500));
    console.log(`== ${sec} ==`);
    console.log(await evalJs(`(() => {
      const d = document.querySelector('.settings-dropdown');
      const active = [...d.querySelectorAll('button')].filter(y => (y.className ?? '').match(/(^| )bg-muted( |$)/)).map(y => y.textContent.trim());
      const host = d.querySelector('.tavern-native-host');
      const text = host ? host.innerText.slice(0, 320) : '(no host)';
      const fallback = [...d.querySelectorAll('p')].filter(p => p.textContent.includes('官方项暂不可用')).length;
      return JSON.stringify({ active, fallbacks: fallback, body: text });
    })()`));
  }
  process.exit(0);
};
setTimeout(() => { console.error("timeout"); process.exit(3); }, 90000);
