...
#
# Uses the fake ssh.exe built from _fake-ssh.cs to take over the remote branch and
# really exercise: config(remoteEnabled) -> ssh args -> child process -> JSONL ->
# remote sidecar -> engine ready. Config.json is temporarily rewritten and restored.
#
# Usage: powershell -ExecutionPolicy Bypass -File scripts\_test_remote_engine.ps1
param(
...
Write-Output "fake ssh: $binDir\ssh.exe"
...
$cfg | Add-Member -Force -NotePropertyName remoteHost -NotePropertyValue "localhost"
...
Write-Output "config.json: remoteEnabled=true host=localhost"
