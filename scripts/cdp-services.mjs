// check which services exist on live ctx for conversation plugin deps
...
ws.onopen = async () => {
  ...
  console.log(await evalJs(`(() => {
    ...
    const out = {};
    for (const d of deps) {
      try { out[d] = c.get ? (c.get(d) !== undefined) : (d in c); } catch (e) { out[d] = "ERR"; }
    }
    // raw prop access fallback
    for (const d of deps) {
      if (out[d] === false) {
        try { out[d + ":prop"] = (c as any)[d] !== undefined; } catch {}
      }
    }
