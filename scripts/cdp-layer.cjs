...
ws.onopen = async () => {
  ...
  console.log(await evalJs(`(() => {
    ...
    const layer = mirror?.querySelector('[data-shell-overlay="true"]');
    if (!layer) return 'no layer';
    const r = layer.getBoundingClientRect();
    return JSON.stringify({
      layerRect: [Math.round(r.x), Math.round(r.y), Math.round(r.width), Math.round(r.height)],
      childCount: layer.childElementCount,
      html: layer.innerHTML.slice(0, 600),
      text: (layer.textContent ?? '').slice(0, 200),
    });
  })()`));
  process.exit(0);
};
