...
- **插件"装上了但引擎起不来"的排查口径**（A 线结论见 `docs/plugin-compat.md`）：插件直接 `import` 的引擎 API 在升级后被删/改名 → **ESM 链接期**报错 → cordis include apply 失败 → 上下文失活 → SDK 侧只看到 `cannot create effect on inactive context`（一句人话，看不出是谁）。**别信"进程还活着"**（CLI 直起不握手是看不出问题的），用 `scripts/_engine_init_probe.mjs`（手写 initialize + 引擎 stderr 全文）或 `_engine_sdk_probe.mjs`（应用路径 READY/FAILED）；`_plugin_versions.mjs` 查 npm 版本。三个脚本都自动备份/恢复 profile，可反复跑。激活插件走 `scripts/_activate_plugin.mjs <pkg>`（官方清单写入语义），**激活前先跑一次 SDK 探针**。
- **通讯（左栏"通讯"图标）= 社区 `dsh-im` 真插件，不是本地假清单**：面板（`components/overlays/MessagingOverlay.tsx`）只读引擎真状态（`plugins.list` / `config.pluginEntries`）+ 一键装渠道适配器（`dsh-im-weixin/-wecom/-feishu/-telegram`），凭据归插件自己的设置分区。桥核心 `dsh-im@1.0.5` 已实测与引擎 0.1.5-rc.1 兼容（SDK 探针 READY）。
