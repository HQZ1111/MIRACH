...
- **本环境模型无法查看图片/截图**。所有 UI 验证、问题诊断、回归测试一律走 **DOM / 文本 / 代码** 手段：
  - 不向模型展示截图或要求"看图"；不要依赖视觉判断。
  - 检查 UI 用元素树、getBoundingClientRect、PowerShell 查窗口句柄/矩形（user32 GetWindowRect）等文本手段。
  - 验证功能看代码路径 + tsc / vite build / 控制台日志。
- 用户要求"看一下界面"时，用代码走查 + DOM 结构回答，不截图。
...
## 架构速览（详见 HANDOFF-2026-08-11~14 系列）
