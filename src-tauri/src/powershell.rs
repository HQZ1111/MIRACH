//! PowerShell 安装器驱动器（整份照搬 hermes `apps/bootstrap-installer/src-tauri/src/powershell.rs`）。
//!
//! 为什么整份搬而不是自己写：hermes 这一层踩过并修好的两个坑，mirach 之前自己写时又踩了一遍 ——
//!   1. `BufReader::lines()` 要求严格 UTF-8；Windows PowerShell 5.1 在非英文系统上按控制台代码页
//!      输出本地化错误（中文机 = GBK），一旦解码失败整行/后续行全丢 —— 连阶段结果 JSON 帧都没了。
//!      这里用「逐字节 `read_until` + [`decode_console_bytes`]（UTF-8 → 代码页回退）」解决。
//!   2. 子进程退出 ≠ 管道 EOF：孙进程继承着写端时 EOF 永远不来（npm 卡住 / 常驻进程持有句柄），
//!      只看 EOF 会把调用方吊死。`pump_child` 以 **进程退出** 为权威终态，尾部排水只给
//!      [`DRAIN_GRACE`] 的宽限，并把「放弃了最后 N 秒输出」显式告诉用户。
//!
//! 保留的语义：行级 stdout/stderr 流式回调、取消信号、`CREATE_NO_WINDOW`、绝对路径解析
//! powershell.exe（PATH 被截断的机器上也能起来）、阶段结果/清单 JSON 解析。
//!
//! Windows 走 `-NoProfile -ExecutionPolicy Bypass -File <script>`；Unix 分支原样保留（mirach 只发 Windows）。
    ...
    FE: FnMut(&str),
{
    ...
    while !(out_done && err_done) {
        tokio::select! {
            line = read_decoded_line(&mut out, &mut out_buf), if !out_done => match line {
                ...
                Err(e) => {
                    eprintln!("[bootstrap] stdout read error: {e}");
                    out_done = true;
                }
            },
            line = read_decoded_line(&mut err, &mut err_buf), if !err_done => match line {
                ...
                Err(e) => {
                    eprintln!("[bootstrap] stderr read error: {e}");
                    err_done = true;
                }
            },
            reaped = child.wait() => {
    ...
    if cancelled {
        eprintln!("[bootstrap] cancellation received — killing child");
    ...
    let status = match status {
        ...
        None => tokio::select! {
            ...
            _ = recv_cancel(cancel_rx) => {
                eprintln!("[bootstrap] cancellation received after EOF — killing child");
                cancelled = true;
                let _ = child.start_kill();
...
/// Spawns the install script with the given args and streams output.
pub async fn run_script(
    ...
    // Pin the child to a stable directory: the app (and its resource dir) can be
    // replaced while an install runs, and a script starting from a deleted cwd
    // emits getcwd/job-working-directory errors at the end of an otherwise
    // successful run. The script's own folder is as stable as it gets.
    if let Some(cwd) = stable_script_cwd(script_path) {
        cmd.current_dir(cwd);
    }
...
fn stable_script_cwd(script_path: &Path) -> Option<&Path> {
...
fn windows_powershell_exe() -> std::path::PathBuf {
    ...
    // PATH 查找兜底（不引 which crate）：PATH 目录里按名字找可执行文件。
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_script_folder() {
        // 脚本所在目录存在时用它当 cwd（脚本目录被换掉时退回 None，不硬撑）
        let dir = std::env::temp_dir();
        let script = dir.join("mirach-install.ps1");
        assert_eq!(stable_script_cwd(&script), Some(dir.as_path()));
        assert_eq!(stable_script_cwd(Path::new("nope/missing.ps1")), None);
    }
    ...
    /// 真机冒烟：用真实安装脚本的 `-Manifest` 走一遍本模块（mirach 侧新增，非 hermes）。
    /// 卡住说明 IO 层在这种运行时/负载下有问题——首装会永远停在"正在读取安装清单"。
    #[cfg(target_os = "windows")]
    #[tokio::test]
    async fn runs_the_real_installer_manifest() {
        let script = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("repo root")
            .join("scripts")
            .join("mirach-install.ps1");
        assert!(script.is_file(), "installer script missing: {}", script.display());
        let sink = StreamSink {
            on_stdout_line: Box::new(|l| println!("[out] {l}")),
            on_stderr_line: Box::new(|l| println!("[err] {l}")),
        };
        let mut cancel = None;
        let result = timeout(
        ...
        .await
        .expect("installer manifest must not hang")
        .expect("run_script ok");
        assert_eq!(result.exit_code, Some(0), "stderr: {}", result.stderr);
        let manifest = parse_manifest(&result.stdout).expect("manifest frame parses");
        assert!(manifest.stages.len() >= 3, "stages: {:?}", manifest.stages);
    }
}
